...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  // вычисление расстояния на плоскости d = корень.кв. ((x1 - x2) ** 2 + (y1 - y2) ** 2)
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const dig1 = Math.sqrt(x1 * x1 + y1 * y1);
  const dig2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(sum / (dig1 * dig2));
}
